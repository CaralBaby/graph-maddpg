...
def make_update_exp(vals, target_vals, central=True):
    ...
    if central:
        ...
        for a in range(agent_n_species):
            for var, var_target in zip(sorted(vals[a], key=lambda v: v.name), sorted(target_vals[a], key=lambda v: v.name)):
                expression.append(var_target.assign(polyak * var_target + (1.0-polyak) * var))
    expression = tf.group(*expression)
    return U.function([], [], updates=[expression])


def p_train(name, make_obs_ph_n, adj_n, act_space_n, neighbor_n_pred, neighbor_n_prey, p_index, p_func, q_func, num_adversaries, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=128, scope="trainer", reuse=None):
    neighbor_n = neighbor_n_pred if name == "adversaries" else neighbor_n_prey
    with tf.variable_scope(scope, reuse=reuse):
        ...
        q = []
        q_reduce_mean = []
        for a in range(0, num_adversaries) if name == "adversaries" else range(num_adversaries, agent_n):
            index = a if name == "adversaries" else a - num_adversaries
            temp = q_func(q_input, 1, scope="q_func_%d" % index, reuse=True, num_units=num_units)[:,0]
            q.append(temp)
            q_reduce_mean += temp
        ...
        update_target_p = make_update_exp(p_func_vars, target_p_func_vars, central=True)
...
def q_train(name, make_obs_ph_n, adj_n, act_space_n, num_adversaries, neighbor_n_pred, neighbor_n_prey, q_func, agent_n, optimizer,
            grad_norm_clipping=None, local_q_func=False, reuse=None, scope="trainer", num_units=64):
    neighbor_n = neighbor_n_pred if name == "adversaries" else neighbor_n_prey
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]
        ...
        obs_ph_n = make_obs_ph_n
        act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action"+str(i)) for i in range(len(act_space_n))]
        target_ph = [tf.placeholder(tf.float32, [None], name="target") for _ in range(agent_n_species)]

        q = []
        q_square = []
        q_input = tf.concat(obs_ph_n + act_ph_n, 1)
        for a in range(agent_n_species):
            temp = q_func(q_input, 1, scope="q_func_%d" % a, num_units=num_units)[:, 0]
            q.append(temp)
        ...
        q_func_vars = [U.scope_vars(U.absolute_scope_name("q_func_%d" % i)) for i in range(agent_n_species)]
        q_loss = [tf.reduce_mean(tf.square(q[i] - target_ph[i])) for i in range(agent_n_species)]
        ...
        optimize_expr = [U.minimize_and_clip(optimizer, loss[i], q_func_vars[i], grad_norm_clipping)
                         for i in range(agent_n_species)]

        # Create callable functions
        train = [U.function(inputs=obs_ph_n + act_ph_n + [target_ph[i]], outputs=loss[i], updates=[optimize_expr[i]])
                 for i in range(agent_n_species)]
        ...
        target_q = []
        for a in range(agent_n_species):
            temp = q_func(q_input, 1, scope="target_q_func_%d" % a, num_units=num_units)[:, 0]
            target_q.append(temp)
        target_q_func_vars = [U.scope_vars(U.absolute_scope_name("target_q_func_%d" % i))
                              for i in range(agent_n_species)]
        update_target_q = make_update_exp(q_func_vars, target_q_func_vars, central=False)
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, p_model, q_model, obs_shape_n, act_space_n, num_adversaries, args, local_q_func=False):
        ...
        self.args = args
        self.neighbor_n_pred = 2
        self.neighbor_n_prey = 2
        ...
        for i in range(self.n):
            ...
            adj_n.append(U.BatchInput([self.neighbor_n_pred if i < num_adversaries else self.neighbor_n_prey,
        ...
        self.q_train, self.q_update, self.q_values, self.target_q_values = q_train(
            ...
            neighbor_n_pred=self.neighbor_n_pred,
            neighbor_n_prey=self.neighbor_n_prey,
            q_func=q_model,
            agent_n=self.n,
    ...
    def update(self, agents, t):
        ...
        target_q_list = [target_q.transpose()[i] for i in range(np.shape(target_q)[1])]

        # train the critic network
        # q_train_input = q_input_obs_n + q_input_act_n + [target_q]
        q_loss = [self.q_train[i](*(q_input_obs_n + q_input_act_n + [target_q_list[i]])) for i in range(len(self.q_train))]
